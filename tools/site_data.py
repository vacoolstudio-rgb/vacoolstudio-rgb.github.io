...
def is_released(app):
    """Вышло ли приложение хоть в одном магазине.

    Вычисляется из идентификаторов, а не хранится отдельным флагом: два поля об
    одном и том же однажды разойдутся, и разойдутся молча — страница скажет
    «скоро», когда приложение уже месяц в продаже, или наоборот.

    Отдельной страницы у невышедшего приложения нет намеренно. Ей нечего
    рассказать: ни ссылки, ни скриншотов, ни того, как оно работает. Для Google
    это «тонкая страница», и оценивает он ими весь сайт, а не только их. Хуже
    другое — такая страница успевает проиндексироваться как обещание, и это
    состояние остаётся в выдаче ещё недели после того, как приложение вышло.
    """
    return bool(app['appstore'] or app['play'])


def released_apps():
    return [a for a in APPS if is_released(a)]


def upcoming_apps():
    return [a for a in APPS if not is_released(a)]
